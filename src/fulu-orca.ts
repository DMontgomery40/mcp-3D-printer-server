...
function normalizePlatform(value: string | undefined): SupportedPlatform {
...
function resolveSlicerPath(explicitPath: string | undefined, platform: SupportedPlatform): string {
...
function defaultRuntimeDir(platform: SupportedPlatform, explicitRuntimeDir?: string): string | undefined {
  ...
  if (platform === "darwin") {
    return (
      process.env.PJARCZAK_MAC_RUNTIME_DIR ||
...
function checkFiles(baseDir: string, names: string[], required = true, scope?: string): FileCheck[] {
...
function platformFileChecks(platform: SupportedPlatform, pluginDir: string, runtimeDir?: string): FileCheck[] {
  if (!pluginDir) {
    return [];
  }
...
function setupCommands(platform: SupportedPlatform, pluginDir: string, runtimeDir?: string): SetupCommand[] {
  if (platform === "darwin") {
  ...
  if (platform === "win32") {
    return [
      ...
      },
      {
        ...
        command: `powershell.exe -NoProfile -ExecutionPolicy Bypass -File ${quotePowerShell(
          path.join(pluginDir, "install_runtime.ps1")
        )} -PackageDir ${quotePowerShell(pluginDir)} -PluginDir ${quotePowerShell(pluginDir)}`,
      },
      {
        label: "verify WSL2 runtime",
        command: `powershell.exe -NoProfile -ExecutionPolicy Bypass -File ${quotePowerShell(
          path.join(pluginDir, "verify_runtime.ps1")
        )} -PackageDir ${quotePowerShell(pluginDir)} -PluginDir ${quotePowerShell(pluginDir)}`,
      },
    ];
  }
...
function defaultBridgeCommand(platform: SupportedPlatform, pluginDir: string, runtimeDir?: string): string | undefined {
  ...
  if (platform === "darwin") {
    ...
    const runtimeHost = path.join(
      runtimeDir || defaultRuntimeDir(platform) || "",
      "pjarczak_bambu_linux_host"
    );
...
function writeJsonRequestFrame(id: number, method: string, payload: Record<string, unknown>): Buffer {
...
function parseBridgeFrames(buffer: Buffer): BridgeFrame[] {
...
async function runBridgeRequests(
  command: string,
        ...
        if (frame.type === FRAME_TYPE_JSON_RESPONSE) {
          try {
            ...
            responses[String(frame.id)] = {
              ok: false,
              error: `Invalid JSON response payload: ${(error as Error).message}`,
...
async function probeFuluBridge(command: string, timeoutMs: number): Promise<Record<string, unknown>> {
  const probe = await runBridgeRequests(
    command,
    [
...
function isReadOnlyBridgeMethod(method: string): boolean {
  return READ_ONLY_BRIDGE_METHODS.has(method) || READ_ONLY_NET_PREFIXES.some((prefix) => method.startsWith(prefix));
}

export async function invokeFuluBridgeRpc(options: FuluBridgeRpcOptions): Promise<Record<string, unknown>> {
  ...
  const bridgeResult = await runBridgeRequests(
    command,
    [{ id: 1, method, payload: options.payload || {} }],
...
export async function inspectFuluOrcaSetup(options: FuluOrcaSetupOptions): Promise<Record<string, unknown>> {
  ...
  const bridgeCommand =
    options.bridgeCommand || defaultBridgeCommand(platform, pluginDir, runtimeDir);
  ...
  let bridgeProbe: Record<string, unknown> | undefined;

  if (options.runBridgeProbe) {
    bridgeProbe = await probeFuluBridge(
      bridgeCommand || "",
